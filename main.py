...
@app.route("/get_group_name", methods=["POST"])
def get_group_name():
    ...
    if op_type == "get_group_name":
        ...
        return jsonify({"status":"success", "gid":group_name})
...
@app.route("/add/<string:target_uid>")
def add_page(target_uid):
    try:
        ...
        return render_template("login/add.html", target_uid=target_uid)
...
def create_group():
    if request.method == "POST":
        ...
        uid = request.cookies.get("uid")
        return render_template("create_group/index.html", uid=uid)
...
@app.route("/profile/<string:uid>")
def profile_page(uid):
    cur.execute(f'SELECT name, icon_url FROM users WHERE id="{uid}"')
    name, icon_url = cur.fetchone()
    print(name, icon_url)
    return render_template("profile/index.html", uid=uid, name=name, icon_url=icon_url)

@app.route("/invite/<string:target_uid>")
def invite_into_group_page(target_uid):
    ...
    for gid, name in datas:
        if gid[0] == "g":
            ...
            pass
    print(datas)
    return render_template("invite/index.html", uid=uid, group_list=zip(gids, names), target_uid=target_uid)
...
def fcm_sw_js():
    return send_file("static/firebase-messaging-sw.js")
...
def group_talk(gid):
    ...
    return render_template("group/index.html", datas=datas, uid=uid, gid=gid, group_name=group_name)

@app.route("/otp_auth", methods=["GET"])
def otp_auth_page():
    uid = request.cookies.get("uid")
    return render_template("register/otp.html", uid=uid)
...
def home_page():
    ...
    return render_template("home/index.html", user_datas=user_datas, group_datas=group_datas, uid=uid)
...
def forgot_password():
    ...
    if type == "first_forgot_password":
        ...
        if exists_mail_address != None:                
            if mail_address not in exists_mail_address:
                return jsonify({"status":"failed", "reason":"Email address is invalid or does not exists"})
            ...
            return jsonify({"status":"failed", "reason": "Any email addresses found"})
